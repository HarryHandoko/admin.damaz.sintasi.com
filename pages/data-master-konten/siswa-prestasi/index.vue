<template>
  <v-card>
    <v-card-title class="pa-4">
      <h2>Master Konten - Siswa Berprestasi</h2>
    </v-card-title>
    <!-- Search & Add -->
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-content/siswa-prestasi/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master-content/siswa-prestasi/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master-content/siswa-prestasi/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-content/siswa-prestasi/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master-content/siswa-prestasi/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-content/siswa-prestasi/update`, {
      ...item
    })
  } catch (error) {
    getData()
