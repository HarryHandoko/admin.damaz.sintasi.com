<template>
  <v-card>
            ...
            <v-row dense>
              ...
              <v-col cols="12" >
                <v-text-field
                  v-model="form.name"
                  label="Nama Bank"
                  :rules="[v => !!v || 'Nama Bank harus diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
              ...
              <v-col cols="12" >
                <v-text-field
                  v-model="form.nama_akun_bank"
                  label="Nama Pemilik Akun Bank"
                  :rules="[v => !!v || 'Nama Pemilik Akun Bank harus diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
              ...
              <v-col cols="12" >
                <v-text-field
                  ...
                  label="No. Rekening"
                  type="number"
                  :rules="[v => !!v || 'No. Rekening harus diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
    ...
    <ConfirmDialog
      ...
      :message="`Apakah Anda yakin ingin menghapus Data ${DataToDelete?.name}?`"
    ...
    >
      <v-card class="mb-4" elevation="2">
        ...
        <v-card-title>
          <v-row>
            <v-col cols="6">
              <span v-if="editingRow === 'nama_akun_bank'+item.id" class="mr-2">
                <v-text-field
                  v-model="item.nama_akun_bank"
                  density="compact"
                  autofocus
                  hide-details
                  @keydown.enter="saveEdit(item)"
              ...
              <span v-else @click="startEdit('nama_akun_bank'+item.id)" style="cursor:pointer;" class="mr-2">
                {{ item.nama_akun_bank ?? '-' }}
              </span>
              |
              <span v-if="editingRow === 'no_rek'+item.id" class="ml-2">
                <v-text-field
                  v-model="item.no_rek"
                  density="compact"
                  autofocus
              ...
              <span v-else @click="startEdit('no_rek'+item.id)" style="cursor:pointer;"  class="ml-2">
                {{ item.no_rek ?? '-' }}
              </span>
            </v-col>
          </v-row>
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-data/bank-account/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master-data/bank-account/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master-data/bank-account/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-data/bank-account/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
    ...
    show.value = true
    message.value = 'Gagal membuat Data.'
  } finally {
    loading.value = false
    getData()
  }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master-data/bank-account/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-data/bank-account/update`, {
      ...item
    })
  } catch (error) {
    getData()
