<template>
  <v-card>
    ...
    <v-dialog v-model="showCreateModal" max-width="600" persistent>
      <v-card>
        <v-card-title class="pa-6 text-center">
          <span class="headline">Form Pendaftar</span>
        </v-card-title>
    ...
    <ConfirmDialog
      ...
      :message="`Apakah Anda yakin ingin menghapus Pendaftar ${DataToDelete?.name}?`"
...
async function saveEdit(item) {
  loading.value = true
  try {
    const response = await $api.post('/master/pendaftar/update', {
      ...item,
    })
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master/pendaftar/get', {
      params: {
        page: pagination.page,
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master/pendaftar/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master/pendaftar/update-data', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
    ...
    show.value = true
    message.value = error.response?.data?.message || 'Gagal membuat Pendaftar.'
  } finally {
    loading.value = false
  }
}
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master/pendaftar/delete`,{
...
async function onToggleActive(item) {
  try {
    ...
    await $api.post(`/master/pendaftar/update/`, {
      ...item
    });
  } catch (error) {
    getData();
