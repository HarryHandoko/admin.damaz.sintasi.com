<template>
  <v-card>
    <v-card-title class="pa-4">
      <h2>Master Konten - Prestasi Sekolah</h2>
    </v-card-title>
    <!-- Search & Add -->
    ...
    <ConfirmDialog
      ...
      :message="`Apakah Anda yakin ingin menghapus Data ${DataToDelete?.prestasi}?`"
  ...
  <v-row class="mt-4" v-if="!loading && items.length">
    ...
    >
      <v-card class="mb-4" elevation="2">
        ...
        />
        <v-card-title>
          <span v-if="editingRow === 'prestasi'+item.id">
            <v-text-field
              v-model="item.prestasi"
              density="compact"
              autofocus
              hide-details
          ...
          <span v-else @click="startEdit('prestasi'+item.id)" style="cursor:pointer;">
            {{ item.prestasi ?? '-' }}
          </span>
        </v-card-title>
        <v-card-actions>
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-content/prestasi-sekolah/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master-content/prestasi-sekolah/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master-content/prestasi-sekolah/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-content/prestasi-sekolah/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
    ...
    show.value = true
    message.value = error.response?.data?.message || 'Gagal membuat Fasilitas.'
  } finally {
    loading.value = false
    getData()
  }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master-content/prestasi-sekolah/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-content/prestasi-sekolah/update`, {
      ...item
    })
  } catch (error) {
    getData()
