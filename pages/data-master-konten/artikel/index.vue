    ...
    <v-dialog v-model="showCreateModal" max-width="700" persistent>
      <v-card>
        <v-card-title class="pa-6 text-center">
          <span class="headline">Form Artikel</span>
        </v-card-title>
        <v-form v-model="valid" @submit.prevent="handleCreateData" ref="formRef">
          <v-card-text style="margin-top: -30px;">
            <v-row dense>
              ...
              <!-- Status artikel -->
              <v-col cols="12" :sm="form.status === 'scheduled' ? 6 : 12">
                <v-select
                  v-model="form.status"
                  :items="[
                    { text: 'Draft', value: 'draft' },
                    { text: 'Jadwalkan', value: 'scheduled' },
                    { text: 'Terbitkan', value: 'published' }
                  ]"
                  item-title="text"
                  item-value="value"
                  label="Status"
              ...
              <!-- Tanggal jadwal (scheduled_at) -->
              <v-col cols="12" sm="6" v-if="form.status === 'scheduled'">
                <v-text-field
                  v-model="form.scheduled_at"
                  label="Tanggal Terbit"
                  type="date"
                  :rules="[v => !!v || 'Tanggal Terbit diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
              <!-- Tag artikel + tombol tambah -->
              <v-col cols="12">
                <v-select
                  v-model="form.tags"
                  :items="tags"
                ...
                <v-row class="mt-2">
                  <v-col cols="8">
                    <v-text-field
                      ...
                      dense
                      hide-details
                      placeholder="Ketik dan klik tambah"
                    />
                  </v-col>
...
// Dummy tags, ganti dengan fetch dari API di mounted
const tags = ref([])
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-content/artikel/update', formData, {
...
function showModalDialog(item) {
  ...
  if (item == null) {
    Object.keys(form).forEach(k => form[k] = (Array.isArray(form[k]) ? [] : null));
    fotoPreview.value = '/no-image.jpg';
  } else {
    Object.keys(form).forEach(k => form[k] = item[k] ?? (Array.isArray(form[k]) ? [] : null));
...
async function getData() {
  loading.value = true;
  try {
    const response = await $api.get('/master-content/artikel/get', {
      params: {
  ...
  try {
    ...
    for (const key in form) {
      if (form[key] !== null && form[key] !== undefined) {
        if (key === 'tags' && Array.isArray(form.tags)) {
    ...
    if(form.id == null){
      await $api.post('/master-content/artikel/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-content/artikel/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
    ...
    show.value = true;
    message.value = error.response?.data?.message || 'Gagal membuat Artikel.';
  } finally {
    loading.value = false;
  }
}
...
async function addNewTag() {
  loading.value = true;
  try {
    await $api.post(`/master-content/artikel/adding-tag`,{
    ...
    show.value = true;
    message.value = error.response?.data?.message || 'Gagal Menyimpan Data.';
  } finally {
    loading.value = false;
  }
}
...
async function handleDeleteData() {
  loading.value = true;
  try {
    await $api.post(`/master-content/artikel/delete`,{
...
async function getTag() {
  loading.value = true
  try {
    const response = await $api.get('/master-content/artikel/get-tag');
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-content/artikel/update`, {
      ...item
    })
  } catch (error) {
    getData()
