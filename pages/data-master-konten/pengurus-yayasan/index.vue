<template>
  <v-card>
    <v-card-title class="pa-4">
      <h2>Master Konten - Pengurus Yayasan</h2>
    </v-card-title>
    <!-- Search & Add -->
    <v-row class="mb-2 pa-4">
      <v-col cols="12" sm="12" md="6">
        <v-text-field
          ...
          label="Cari"
          placeholder="Cari Pengurus Yayasan"
          @keyup.enter="getData"
          clearable
        />
      </v-col>
      <v-col cols="12" sm="12" md="6">
        ...
        >
          ...
          Tambah Pengurus Yayasan
        </v-btn>
      </v-col>
    </v-row>

    <!-- Dialog tambah/edit Pengurus Yayasan -->
    <v-dialog v-model="showCreateModal" max-width="600" persistent>
      <v-card>
        <v-card-title class="pa-6 text-center">
          <span class="headline">Form Pengurus Yayasan</span>
        </v-card-title>
        <v-form v-model="valid" @submit.prevent="handleCreateData" ref="formRef">
          <v-card-text style="margin-top: -30px;">
            <v-row dense>
              ...
              <v-col cols="12" >
                <v-text-field
                  v-model="form.pengurus"
                  label="Nama Pengurus"
                  :rules="[v => !!v || 'Nama Pengurus harus diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
  ...
  <v-row class="mt-4" v-if="!loading && items.length">
    ...
    >
      <v-card class="mb-4" elevation="2">
        ...
        />
        <v-card-title>
          <span v-if="editingRow === 'pengurus'+item.id">
            <v-text-field
              v-model="item.pengurus"
              density="compact"
              autofocus
              hide-details
          ...
          <span v-else @click="startEdit('pengurus'+item.id)" style="cursor:pointer;">
            {{ item.pengurus ?? '-' }}
          </span>

          <br>
          <span v-if="editingRow === 'jabatan'+item.id">
            <v-text-field
          ...
          <span v-else @click="startEdit('jabatan'+item.id)" style="cursor:pointer;">
           <b> <i>{{ item.jabatan ?? '-' }}</i></b>
          </span>
        </v-card-title>
        <v-card-actions>
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-content/pengurus/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master-content/pengurus/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master-content/pengurus/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-content/pengurus/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
    ...
    show.value = true
    message.value = error.response?.data?.message || 'Gagal membuat pengurus.'
  } finally {
    loading.value = false
    getData()
  }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master-content/pengurus/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-content/pengurus/update`, {
      ...item
    })
  } catch (error) {
    getData()
