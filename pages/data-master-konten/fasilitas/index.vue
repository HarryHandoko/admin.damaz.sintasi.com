    ...
    <v-dialog v-model="showCreateModal" max-width="600" persistent>
      <v-card>
        <v-card-title class="pa-6 text-center">
          <span class="headline">Form Fasilitas</span>
        </v-card-title>
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-content/fasilitas/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master-content/fasilitas/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master-content/fasilitas/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-content/fasilitas/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master-content/fasilitas/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-content/fasilitas/update`, {
      ...item
    })
  } catch (error) {
    getData()
