<template>
  <div>
    ...
    <v-row v-if="is_email_verifikasi == 1">
      <v-col cols="12">
        <VCard class="pa-5">
          <v-row>
            <v-col cols="12" md="5">
              <v-text-field label="Cari Nama Siswa" v-model="form.search" dense outlined></v-text-field>
            </v-col>
      ...
      <v-col v-for="(item, index) in dataRegister" :key="index" cols="12" md="4" style="cursor: pointer;">
        <v-card class="rounded-2xl elevation-10" style="box-shadow: 0 10px 24px -8px #60a5fa33;">
          <!-- Tombol Delete di kanan atas -->
          <v-btn icon class="ma-2 mb-4" color="red" style="position: absolute; top: 0; right: 0; z-index: 10;color:red"
            @click.stop="deleteRegister(item)" v-if="item.is_daftar_ulang == 0">
            <v-icon size="30" v-if="!loading">bx bx-trash</v-icon>
            <v-progress-circular v-else indeterminate color="primary" size="30" />
          </v-btn>


          <v-btn icon class="ma-2" color="red"
            ...
            @click.stop="printData(item)" v-if="item.is_submit == '1' && item.status_pendaftaran == 'P01'">
          ...
          <v-row align="center" no-gutters class="pa-5" @click="createRegister(item)">
            <v-col cols="12" md="3" class="flex justify-center items-center text-center">
              <v-avatar size="70" class="elevation-5"
                style="border: 2px solid #38bdf8; box-shadow: 0 4px 18px 0 #38bdf833;">
                <v-img :src="item.siswa.foto_siswa != null ? item.siswa.foto_siswa : '/favicon.ico'" cover
                  alt="Avatar" />
              </v-avatar>
            </v-col>
          ...
          <v-col cols="12" v-if="item.is_daftar_ulang == '0'" :class="item.status_pendaftaran == 'P00'
              ? 'pa-2 bg-warning'
          ...
          <v-col cols="12" v-if="item.is_daftar_ulang == '1'" :class="item.registrasi_ulang?.status_pembayaran == '00'
              ? 'pa-2 bg-warning'
        ...
        <v-form v-model="valid" @submit.prevent="handleCreateData">
          <v-card-text style="margin-top: -30px;">
            ...
            <v-row v-if="step == 0">
              ...
              <v-col cols="12">
                <div class="d-flex justify-space-between align-center">
                  ...
                  <span @click="step = 1" v-if="form.dataPPDB?.status_pendaftaran == 'P00'"
                    style="cursor : pointer;color:blue">Show Detail</span>
                </div>
              ...
              <v-col cols="12" class="mt-4" style="border-top:1px solid #d9d9d9">
                <div class="d-flex justify-space-between align-center">
                  ...
                  <span @click="step = 2" v-if="form.dataPPDB?.status_pendaftaran == 'P00'"
                    style="cursor : pointer;color:blue">Show Detail</span>
                </div>
              ...
              <v-col cols="12" class="mt-4" style="border-top:1px solid #d9d9d9">
                <div class="d-flex justify-space-between align-center">
                  <b>Alamat</b>
                  <span @click="step = 3" v-if="form.dataPPDB?.status_pendaftaran == 'P00'"
                    style="cursor : pointer;color:blue">Show Detail</span>
                </div>
              ...
              <v-col cols="12" class="mt-4" style="border-top:1px solid #d9d9d9">
                <div class="d-flex justify-space-between align-center">
                  <b>Data Orang Tua</b>
                  <span @click="step = 4" v-if="form.dataPPDB?.status_pendaftaran == 'P00'"
                    style="cursor : pointer;color:blue">Show Detail</span>
                </div>
              ...
              <v-col cols="12" class="mt-4" style="border-top:1px solid #d9d9d9">
                <div class="d-flex justify-space-between align-center">
                  ...
                  <span @click="step = 5" v-if="form.dataPPDB?.status_pendaftaran == 'P00'"
                    style="cursor : pointer;color:blue">Show Detail</span>
                </div>
            ...
            <v-row v-if="step == 2">
              ...
              <v-col cols="12" sm="12" stlye="padding:0">
                <div v-if="fotoPreviewFotoSiswa" class="mt-2 text-center">
                  <img :src="fotoPreviewFotoSiswa" alt="Preview Foto"
                    style="width: 120px; height: 120px; object-fit: cover; border-radius: 5%; border: 2px solid #eee;" />
                </div>
                <v-file-input label="Upload Foto" accept="image/*" show-size @change="handleFotoChangeFotoSiswa"
                  class="mb-2" />
              </v-col>
              <v-col cols="12" md="6">
              ...
              <v-col cols="12">
                <v-switch v-model="form.kebutuhan_spesial" label="Apakah memiliki kebutuhan khusus?" color="primary"
                  inset></v-switch>
              </v-col>
              ...
              <v-col cols="12" v-if="form.dataSekolah != null ? form.dataSekolah.is_need_nem == '1' : false">
                ...
                <v-file-input label="Upload Ijazah Terakhir" show-size v-modal="form.file_raport" class="mb-2"
              ...
              <v-col cols="12" v-if="form.dataSekolah != null">
                ...
                <v-file-input label="Upload Akte Lahir" show-size v-modal="form.file_akte_lahir" class="mb-2"
                  @change="handleFileAkte" />
              </v-col>
              <v-col cols="12" v-if="form.dataSekolah != null">

                <div class="mb-2" v-if="form.dataPPDB.file_kartu_keluarga != null">
                  <a :href="form.dataPPDB.file_kartu_keluarga" target="_blank"><v-icon>bx-file</v-icon> Kartu
                    Keluarga</a>
                </div>
                <v-file-input label="Upload Kartu Keluarga" show-size v-modal="form.file_kartu_keluarga" class="mb-2"
                  @change="handleFileKK" />
              </v-col>

              <v-col cols="12">
                <v-select v-model="form.award" :items="['Tidak ada', 'Ada']" label="Apakah memiliki prestasi?" required
                  return-object="false" />
              </v-col>
              ...
              <v-col cols="12" sm="12" stlye="padding:0" v-if="(form.award == 'Ada')">
                ...
                <v-file-input label="Upload Foto" accept="image/*" show-size @change="handleFotoChange" class="mb-2" />
              </v-col>
            ...
            <v-row v-if="step == 4">
              ...
              <v-col cols="12" md='6'>
                <v-text-field v-model="form.no_telepon_ayah" label="No Telepon" required />
              </v-col>

              <v-col cols="12" md='6'>
                <v-text-field v-model="form.no_hp_ayah" label="No Handphone" required />
              </v-col>

              <v-col cols="12" md="6">
                <v-select v-model="form.pendidikan_terakhir_ayah" :items="[
                  'SD / MI',
                  'SMP / MTs',
                  'SMA / MA',
                  'SMK / MAK',
              ...
              <v-col cols="12">
                <v-select v-model="form.penghasilan_ayah" :items="[
              ...
              <v-col cols="12">
                <v-checkbox v-model="form.is_same_address_ayah" label="Apakah alamat sama dengan siswa ?"
              ...
              <v-col cols="12" md='6'>
                <v-text-field v-model="form.no_telepon_ibu" label="No Telepon" required />
              </v-col>

              <v-col cols="12" md='6'>
                <v-text-field v-model="form.no_hp_ibu" label="No Handphone" required />
              </v-col>
              <v-col cols="12" md="6">
                <v-select v-model="form.pendidikan_terakhir_ibu" :items="[
                  'SD / MI',
                  'SMP / MTs',
                  'SMA / MA',
                  'SMK / MAK',
              ...
              <v-col cols="12">
                <v-select v-model="form.penghasilan_ibu" :items="[
                  'Kurang dari Rp 1.000.000',
                  'Rp 1.000.000 – Rp 2.000.000',
                  'Rp 2.000.000 – Rp 3.000.000',
                  'Rp 3.000.000 – Rp 5.000.000',
              ...
              <v-col cols="12">
                <v-checkbox v-model="form.is_same_address_ibu" label="Apakah alamat sama dengan siswa ?"
              ...
              <v-col cols="12" md='6'>
                <v-text-field v-model="form.no_hp_wali" label="No Handphone" required />
              </v-col>
              <v-col cols="12" md="6">
                <v-select v-model="form.pendidikan_terakhir_wali" :items="[
                  'SD / MI',
                  'SMP / MTs',
                  'SMA / MA',
                  'SMK / MAK',
              ...
              <v-col cols="12">
                <v-select v-model="form.penghasilan_wali" :items="[
                  'Kurang dari Rp 1.000.000',
                  'Rp 1.000.000 – Rp 2.000.000',
                  'Rp 2.000.000 – Rp 3.000.000',
                  'Rp 3.000.000 – Rp 5.000.000',
              ...
              <v-col cols="12">
                <v-checkbox v-model="form.is_same_address_wali" label="Apakah alamat sama dengan siswa ?"
            ...
            <v-row v-if="step == 5">
              ...
              <v-col cols="12" style="border-top:1px solid #d9d9d9">
                ...
                <v-row style="margin-bottom: 12px;">
                  <v-col cols="12" md="6" v-if="dataBank && dataBank.length" v-for="(bank, index) in dataBank"
                    :key="index">

                    <v-card class="text-center" style="width: 200px; height: 150px;">
                      <v-img :src="bank.image" cover width="100%" height="100%"></v-img>
                    </v-card>
              ...
              <v-col cols="12" style="border-top:1px solid #d9d9d9">
                Status Pembayaran : <span
                  :style="form.payment_status == 'Menunggu' ? 'color:blue' : form.payment_status == 'Pembayaran Berhasil' ? 'color:green' : 'color:red'"><b>{{
                    form.payment_status }}</b></span>
              </v-col>

              <v-col cols="12" sm="12" stlye="padding:0">
                <div v-if="fotoPreviewBukti" class="mt-2 text-center">
                  <img :src="fotoPreviewBukti" alt="Preview Foto"
                    style="width: auto; height: 120px; object-fit: cover; border-radius: 5%; border: 2px solid #eee;" />
                </div>
                <v-file-input label="Upload Foto" accept="image/*" show-size @change="handleFotoChangeBukti"
                  class="mb-2" />
              </v-col>
            ...
            <v-row v-if="step == 6" justify="center" align="center">
              <v-col cols="12" class="text-center">
                <!-- Animasi GIF sukses -->
                <img src="/images/success.gif" alt="Success" style="max-width: 200px; margin-bottom: 20px;" />
            ...
            <v-row v-if="step == 7">
              ...
              <v-col cols="12" style="border-top:1px solid #d9d9d9"
                ...
                Status Pembayaran : <span
                  :style="formDaftarUlang.data.status_pembayaran == '00' ? 'color:blue' : formDaftarUlang.data.status_pembayaran == '01' ? 'color:green' : 'color:red'"><b>{{
                    formDaftarUlang.data.status_pembayaran == '00' ? 'Menunggu Verifikasi' :
                      formDaftarUlang.data.status_pembayaran == '01' ? 'Pembayaran Berhasil' : 'Pembayaran ditolak'
                    }}</b></span>
              </v-col>

              <v-col cols="12" sm="12" stlye="padding:0">
                <div v-if="foto_bukti_regis_ulang" class="mt-2 text-center">
                  <img :src="foto_bukti_regis_ulang" alt="Preview Foto"
                    style="width: auto; height: 120px; object-fit: cover; border-radius: 5%; border: 2px solid #eee;" />
                </div>
                <v-file-input v-if="formDaftarUlang.data?.bukti_pembayaran == null" label="Upload Foto" accept="image/*"
                  show-size @change="handleFotoChangeBuktiRegisUlang" class="mb-2" />
              </v-col>
            ...
            <v-row class="d-none d-md-flex" v-if="step != 6 && step != 0 && step != 7">
              <v-col cols="12" class="text-right">
                <v-row>
                  <v-col cols="12" md="4">
                    <v-btn color="error" variant="flat" class="mr-2" @click="showCreateModal = false; getDataRegister()"
                      block>Batal</v-btn>
                  </v-col>

                  <v-col cols="12" md="4">
                    <v-btn color="secondary" variant="flat"
                      :disabled="loading || (form.dataPPDB?.is_submit == '1' ? false : step == 1)" block class="mr-2"
                      :loading="loading"
                      @click="form.dataPPDB?.is_submit == '1' ? (step = 0, getDetail()) : (step = step - 1, getDetail())">
                      Kembali
                    </v-btn>
                  </v-col>
                  ...
                  <v-col cols="12" md="4">
                    <v-btn color="primary" variant="flat" :disabled="loading" type="submit" :loading="loading" block>
                      {{ step < 5 ? 'Simpan dan lanjut' : 'Kirim' }} </v-btn>
                  </v-col>
                </v-row>
              </v-col>
            </v-row>
            ...
            <v-row v-if="step == 6 || step == 0">
              <v-col cols="12" class="text-right">
                <v-row>
                  <v-col>
                    <v-btn color="primary" variant="flat" class="mr-2" :loading="loading"
                      @click="showCreateModal = false; getDataRegister()" block>Tutup</v-btn>
                  </v-col>

                  <v-col
                    v-if="form.dataPPDB?.status_pendaftaran == 'P01' && (form.dataPPDB?.is_need_test == '1' ? form.dataPPDB?.status_test == '01' : true) && (form.dataPPDB.registrasi_ulang != null ? form.dataPPDB.registrasi_ulang?.status_pembayaran == '00' : true)">
                    <v-btn color="warning" variant="flat" class="mr-2" :loading="loading"
                      @click="daftarUlang(form.dataPPDB)" block>
                      Daftar Ulang
                    </v-btn>
                  </v-col>
                </v-row>
            ...
            <v-row v-if="step == 7">
              <v-col cols="12" class="text-right">
                <v-row>
                  <v-col
                    :cols="form.dataPPDB?.status_pendaftaran == 'P01' && formDaftarUlang.data?.bukti_pembayaran == null ? '6' : '12'">
                    <v-btn color="secondary" variant="flat" class="mr-2" :loading="loading" @click="step = 0"
                      block>Kembali</v-btn>
                  </v-col>

                  <v-col cols="6" v-if="formDaftarUlang.data?.bukti_pembayaran == null">
                    <v-btn color="primary" variant="flat" :disabled="loading" @click="submitPayRegistrasiUlang"
    ...
    <ConfirmDialog :modelValue="showConfirm" :title="titleConfirm" :message="messageConfirm" :color="color"
      @confirm="showModal(dataConfirm)" @cancel="showConfirm = false" />


    <ConfirmDialog :modelValue="showConfirmDelete" :title="titleConfirmDelete" :message="messageConfirmDelete"
      :color="colorDelete" @confirm="handleDelete(dataConfirmDelete)" @cancel="showConfirmDelete = false" />
