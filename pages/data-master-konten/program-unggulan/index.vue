<template>
  <v-card>
    <v-card-title class="pa-4">
      <h2>Master Konten - Program Unggulan</h2>
    </v-card-title>
    <!-- Search & Add -->
              ...
              <v-col cols="12" sm="12">
                <div v-if="fotoPreview" class="mt-2 text-center">
                  <img
                    :src="fotoPreview"
                    alt="Preview Foto"
                    style="width: 120px; height: 120px; object-fit: cover; border-radius: 2%; border: 2px solid #eee;"
                  />
                </div>
                <v-file-input
                  label="Upload Foto"
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/master-content/program-unggulan/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/master-content/program-unggulan/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/master-content/program-unggulan/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/master-content/program-unggulan/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/master-content/program-unggulan/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/master-content/program-unggulan/update`, {
      ...item
    })
  } catch (error) {
    getData()
