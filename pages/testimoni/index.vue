<template>
  <v-card>
    ...
    <v-dialog v-model="showCreateModal" max-width="600" persistent>
      <v-card>
        <v-card-title class="pa-6 text-center">
          <span class="headline">Form Testimoni</span>
        </v-card-title>
            ...
            <v-row dense>
              ...
              <v-col cols="12" >
                <v-text-field
                  v-model="form.nama"
                  label="Nama Lengkap"
                  :rules="[v => !!v || 'Nama Lengkap harus diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
              ...
              <v-col cols="12" >
                <v-textarea
                  ...
                  label="Testimoni"
                  :rules="[v => !!v || 'Testimoni harus diisi']"
                  required
                  class="mb-2"
                />
              </v-col>
    ...
    <ConfirmDialog
      ...
      :message="`Apakah Anda yakin ingin menghapus Data ${DataToDelete?.nama}?`"
  ...
  <v-row class="mt-4" v-if="!loading && items.length">
    ...
    >
      <v-card class="mb-4" elevation="2">
        ...
        />
        <v-card-title>
          <span v-if="editingRow === 'nama'+item.id">
            <v-text-field
              v-model="item.nama"
              density="compact"
              autofocus
          ...
          <span v-else @click="startEdit('nama'+item.id)" style="cursor:pointer;">
            <b>{{ item.nama ?? '-' }}</b>
          </span>
          <br>
          ...
          <span v-else @click="startEdit('jabatan'+item.id)" style="cursor:pointer;font-size:15px">
            {{ item.jabatan ?? '-' }}
          </span>
        </v-card-title>

        <v-col class="p-2">
          <span v-if="editingRow === 'testimoni'+item.id">
            <v-text-field
              v-model="item.testimoni"
              density="compact"
              autofocus
              hide-details
              @keydown.enter="saveEdit(item)"
          ...
          <span v-else @click="startEdit('testimoni'+item.id)" style="cursor:pointer;font-size:15px">
            <i>
              "{{ item.testimoni ?? '-' }}"
            </i>
          </span>
...
async function saveEdit(item) {
  ...
  try {
    ...
    const response = await $api.post('/testimoni/update', formData, {
...
async function getData() {
  loading.value = true
  try {
    const response = await $api.get('/testimoni/get', {
      params: {
...
async function handleCreateData() {
  ...
  try {
    ...
    if(form.id == null){
      await $api.post('/testimoni/post', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }else{
      await $api.post('/testimoni/update', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
    }
...
async function handleDeleteData() {
  loading.value = true
  try {
    await $api.post(`/testimoni/delete`,{
...
async function onToggleActive(item) {
  try {
    await $api.post(`/testimoni/update`, {
      ...item
    })
  } catch (error) {
    getData()
