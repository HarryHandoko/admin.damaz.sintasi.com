        ...
        <template #item.actions="{ item }">
          <div style="min-width: 200px">
            ...
            <VBtn
              icon
              color="success"
              :disabled="item.status_pembayaran == '01'"
              @click="approvalConfirm(item, 'Approve')"
              :loading="loading"
              class="ml-2"
            >
            ...
            <VBtn
              icon
              color="error"
              :disabled="item.status_pembayaran == '02'"
    ...
    <VDialog v-model="dialogDetail" max-width="700px" persistent>
      <VCard class="pa-4">
        <VCardTitle class="text-h6">
          <VRow>
            <VCol cols="10"> Detail Registrasi Ulang </VCol>
            <VCol cols="2" class="text-right">
              <v-icon style="cursor: pointer" @click="dialogDetail = false"
                >bx bx-x</v-icon
              >
            </VCol>
          </VRow>
        ...
        <VCardText v-if="selectedItem">
          <VContainer>
            <VRow>
              <VCol cols="12" md="6">
                <p>
                  <strong>Kode Registrasi Ulang:</strong>
                  {{ selectedItem.code_registrasi_ulang }}
                </p>
                <p>
                ...
                <p>
                  ...
                  {{ selectedItem.register.nama_depan }}
                  {{ selectedItem.register.nama_belakang }}
                </p>
                ...
                <p>
                  ...
                  {{ selectedItem.register.no_handphone }}
                </p>
              </VCol>
              <VCol cols="12" md="6">
                <p>
                ...
                <p>
                  <strong>Biaya Uang Pangkal:</strong> Rp
                  {{ Number(selectedItem.biaya_pendaftaran).toLocaleString() }}
                </p>
                <p>
                  <strong>Diskon Biaya Uang Pangkal:</strong> Rp
                  {{
                    Number(selectedItem.diskon_uang_pangkal).toLocaleString()
                  }}
                </p>
                <p>
                  <strong>Total Biaya Uang Pangkal:</strong> Rp
                  {{
                    Number(
                      selectedItem.biaya_pendaftaran -
                        selectedItem.diskon_uang_pangkal,
                    ).toLocaleString()
                  }}
                </p>
                <p>
                  <strong class="mr-2">Status Pembayaran:</strong>
                  <VChip
                    :color="
                      selectedItem.bukti_pembayaran != null
                        ? 'success'
                        : 'error'
                    "
                    dark
        ...
        <VCardActions>
          <v-row>
            <v-col cols="6">
              <v-btn
                color="success"
                :disabled="selectedItem.status_pembayaran == '01'"
                variant="flat"
            ...
            <v-col cols="6">
              <v-btn
                color="error"
                :disabled="selectedItem.status_pembayaran == '02'"
                variant="flat"
...
const headers = [
  ...
  {
    title: "Kode Daftar Ulang",
    value: "code_registrasi_ulang",
    width: "200px",
  },
...
async function HandleApproval(data, type) {
  ...
  try {
    const response = await $api.post(
      "/register-ppdb/pendaftaran-ulang-approval",
      {
